...
export default function AajTakNavbar() {
    ...
    return (
        <>
            ...
            <div className="w-full bg-white h-[60px] flex justify-center sticky top-4 z-50 font-sans shadow-sm border-b border-gray-200">
            <div className="w-full max-w-[1280px] px-4 flex items-center h-full">

                {/* Mobile Menu */}
                <button
                    onClick={() => setOpen(!open)}
                    className="text-gray-800 mr-4 xl:hidden hover:text-[#da0000] hover:scale-110 transition-all duration-300"
                ...
                {/* Logo */}
                <Link href="/" title="Dainik Sabse Tej Home" className="flex items-center justify-center flex-shrink-0 mr-6 md:mr-8 group cursor-pointer h-full z-50 py-1.5">
                    <Image
                        src={logo}
                        alt="Dainik Sabse Tej Logo"
                        title="Dainik Sabse Tej"
                        width={180}
                        height={38}
                        priority
                        className="h-[38px] w-auto object-contain transition-transform duration-300 transform-gpu group-hover:scale-105"
                        ...
                        return (
                            <div key={link.name} className="relative group h-full flex items-center">
                                <Link
                                    href={link.path}
                                    title={link.name}
                                    className={`flex items-center text-[15px] font-bold px-4 h-full transition-all duration-300 hover:bg-gray-50 hover:text-[#da0000] ${isActive
                                        ? "text-[#da0000]"
                                        : "text-gray-800"
                                        }`}
                                >
                                    {/* Animated Bottom Border */}
                                    <div className={`absolute bottom-0 left-0 w-full h-[4px] bg-[#da0000] transition-transform duration-300 origin-left ${isActive ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'}`}></div>
                                    {link.name}
                                    {link.hasDropdown && (
                                ...
                                {link.hasDropdown && link.subLinks && (
                                    <div className="absolute top-[60px] left-0 w-48 bg-white border border-gray-100 shadow-xl rounded-b-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 flex flex-col z-50 overflow-hidden">
                                        {link.subLinks.map(sub => (
                                            <Link key={sub.name} href={sub.path} title={sub.name} className="px-5 py-3.5 text-[15px] text-gray-800 hover:bg-gray-50 hover:text-[#da0000] font-bold border-b border-gray-50 last:border-0 transition-colors">
                                                {sub.name}
                                            </Link>
                                        ))}
                                    </div>
                ...
                {/* Right Side Icons */}
                <div className="flex items-center gap-2 sm:gap-4 xl:gap-6 text-gray-800 ml-auto">
                    ...
                    <select 
                        ...
                        className="bg-gray-100 text-gray-800 font-bold py-1 px-1.5 sm:py-1.5 sm:px-3 rounded border border-gray-200 focus:outline-none focus:border-[#da0000] text-[11px] sm:text-sm cursor-pointer hover:bg-gray-200 transition-all appearance-none"
                    ...
                    <Link href="/epaper" title="ई-पेपर" aria-label="E-paper" className="hidden md:flex hover:text-[#da0000] hover:scale-110 transition-all duration-300">
                        <Newspaper size={22} />
                    </Link>

                    {/* X */}
                    <a href="https://x.com/dainiksabsetej" title="Follow us on X" aria-label="Follow us on X" target="_blank" rel="noopener noreferrer" className="hidden md:block hover:text-[#da0000] hover:scale-110 transition-all duration-300">
                        <svg
                    ...
                    {/* YouTube */}
                    <a href="https://www.youtube.com/@hbnnews24x7" title="Subscribe on YouTube" aria-label="Subscribe on YouTube" target="_blank" rel="noopener noreferrer" className="hidden md:block hover:text-[#da0000] hover:scale-110 transition-all duration-300">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="22"
                            height="22"
                    ...
                    <div className="relative" ref={notifRef}>
                        <button 
                            ...
                            title="Notifications"
                            className="hover:text-[#da0000] hover:scale-110 transition-all duration-300 relative"
                        >
                            <Bell size={22} />
                            {unreadCount > 0 && (
                    ...
                    <div className="relative flex items-center">
                        <button 
                            ...
                            className="hover:text-[#da0000] hover:scale-110 transition-all duration-300 bg-gray-100 p-2 rounded-full hover:bg-gray-200"
                    ...
                    <div className="absolute top-[60px] left-0 w-64 bg-white/95 backdrop-blur-xl shadow-2xl z-50 rounded-br-2xl xl:hidden border border-gray-100 overflow-hidden flex flex-col">
                        ...
                        <div className="px-6 py-4 flex flex-col gap-4 bg-gray-50 border-t border-gray-100/50">
                            <div className="flex items-center justify-between px-4 text-gray-800 mt-2">
                                ...
                                <a href="https://x.com/dainiksabsetej" title="Follow us on X" target="_blank" rel="noopener noreferrer" className="hover:text-[#ff3b22] hover:scale-110 transition-all duration-300">
