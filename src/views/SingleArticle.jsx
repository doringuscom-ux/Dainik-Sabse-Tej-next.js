...
export default function SingleArticle({ initialArticle }) {
    ...
    const handleShare = async () => {
        const shareData = {
            title: article?.title || 'दैनिक सबसे तेज़ News',
            text: article?.metaDescription || 'Read this news on दैनिक सबसे तेज़',
            ...
            <div className="w-full md:w-3/4 flex flex-col gap-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-200 pb-4 gap-4">
                    ...
                    <div className="flex items-center gap-4 text-gray-500">
                        <a href="https://www.google.com/preferences/source?q=dainiksabsetej.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 border border-[#da0000] bg-white rounded-md px-2.5 py-[3px] hover:bg-red-50 transition-colors mr-1 sm:mr-2" title="Follow on Google News">
                        ...
                        <a href="https://www.facebook.com/dainiksabsetej" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-700 transition-colors" title="Facebook">
                    ...
                    <div className="mt-8 bg-gray-50 border border-gray-200 rounded-lg p-5 flex flex-col sm:flex-row items-center sm:items-start gap-4 shadow-sm">
                        ...
                        <div className="flex flex-col text-center sm:text-left">
                            <h4 className="font-bold text-lg text-gray-900 mb-1">
                                About the Author: <Link href={`/reporter/${(article.author || 'Admin').toLowerCase().replace(/\s+/g, '-')}`} className="hover:text-[#da0000] hover:underline transition-colors">{article.author || 'दैनिक सबसे तेज़ Desk'}</Link>
                            </h4>
                            <p className="text-gray-600 text-[15px] leading-relaxed">
                                {article.author && !['admin', 'एडमिन'].includes(article.author.toLowerCase())
                                    ? `${article.author} is a dedicated journalist and reporter for दैनिक सबसे तेज़, committed to bringing you the most accurate and fastest news updates from ground zero.`
                                    : 'दैनिक सबसे तेज़ Editorial Desk comprises a team of experienced journalists and editors dedicated to providing fast, verified, and unbiased news to our readers.'}
                            </p>
                        </div>
                    </div>
                )}
